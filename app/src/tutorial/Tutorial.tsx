    ...
    },
    {
      ...
      focus: () => ({
        locations: [
          ...
          this.location(LocationType.FirstPlayerCardSpot).location
        ],
        staticItems: playerBoardDescription.staticItems.filter(elem => elem.location.player === me).map(item => ({ type: MaterialType.PlayerBoard, item }))
      }),
      move: {
        filter: (move) =>
          isMoveItem(move)
